...
third_RT_Mnk = zeros(length(mnkShifts),3);
split_RT_Mnk = zeros(length(mnkShifts),2);
for i = 1:length(mnkShifts)
    currentShift_len = size(mnkShifts{1,i},1);
    splitThresh = stepThr_Mnk(i,1);
    split1 = currentShift_len/3;
    split2 = (currentShift_len/3)*2;
    grpT1 = mean(mnkShifts{1,i}(1:split1,8));
    grpT2 = mean(mnkShifts{1,i}(split1:split2,8));
    grpT3 = mean(mnkShifts{1,i}(split2:end,8));
    splt1 = mean(mnkShifts{1,i}(1:splitThresh,8));
    splt2 = mean(mnkShifts{1,i}(splitThresh:end,8));
