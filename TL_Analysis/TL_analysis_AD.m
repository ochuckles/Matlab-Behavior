...
[~, ~, raw] = xlsread('X:\Ian\TL_Analysis\TL_analysis_clean.xlsx','Sheet1','A2:G162');
...
data_file = cellVectors(:,1);
sav_file = data(:,1);
item_file = cellVectors(:,2);
cch = cellVectors(:,3);
pre_post = cellVectors(:,4);
monkey = cellVectors(:,5);
dc = cellVectors(:,6);
...
for i=1:sets
  ...
  for j=1:sets
     ...
     TL_fileName  = strcat(dataFolder, string(cellVectors{j,1}));
    %import the data
