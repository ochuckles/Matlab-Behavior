...
mondir = ['\\towerexablox.wanprc.org\Buffalo\Cortex Data\' monkey '\'];
imgdir = '\\towerexablox.wanprc.org\Buffalo\eblab\Cortex Programs\DMS PS Yassa\DMSPS Training\Training Image Sets\';
...
for fillop=1:size(filelist,2)
    ...
    % =====================================================================
    % Only look at the first p percent of the dms trials (as a decimal)
    p = 0.50;
    % =====================================================================
    
    event_arr = event_arr(:,1:round(size(event_arr,2)*(1-p)));
    time_arr = time_arr(:,1:round(size(time_arr,2)*(p)));
